...
class AbstractStateDataGenerator():
    ...
    def reset_state(self, times=50):
        ...
        if len(self.env.get_higher_token()) > 1:
            if times == 0:
                ...
                return -1
            return self.reset_state(times-1)
    ...
    def generate_data(self, num_samples):
        ...
        for sample in range(num_samples):
            ...
            next_state, higher_index = self.step(abstract_action, subgoal)
    ...
    def get_abstract_action(self, higher_index):
        ...
        relative_action_ = base_config["relative_actions"][higher_action_idx][0]
    ...
    def get_abstract_state(self):
        ...
        record_init_state = self.env.state
        for action in action_sequence:
            next_state, _, _ = self.env.step(action, record_step=False)
            action_ = np.zeros((4))
            action_[action] = 1
        ...
        self.env.state = record_init_state
    ...
    def step(self, abstract_action, subgoal):
        # First run the policy with the abstract action
        self.env, actions, rewards = self.action_model.execute_policy(self.env, torch.flatten(abstract_action))

        # Then take an additional step with the relative action
        print("Relative action : ", subgoal)
        relative_action = np.argmax(subgoal)
        self.env.step(relative_action)
